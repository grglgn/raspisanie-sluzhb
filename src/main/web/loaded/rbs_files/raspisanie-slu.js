...
function drawGrid(useCached){
  ...
  $('#raspGrid').w2grid({
          ...
          columns: [
              ...
              { field: 'dsc', caption: 'Описание', size: '35%',attr: "align=left"}
          ],
...
function constrAzBtn(ind){
    var rec = elo.bsData[ind];
    var b='<a href="https://azbyka.ru/days/'+rec['buDate']+ '" class="azLink" target="_newwin">АЗ</a>';
    return b;
}
...
function prepareBSData(){
  ...
  while (curDt <= endDt){
	  ...
	  if (wd==1){
		  //remove '<date>.'
		  dd['dsc'] = dd['dsc'].substring(dd['dsc'].indexOf('.')+1);
	  } else if (wd) {
		  //dd['dsc'] = dd['dsc'].substring(dd['dsc'].indexOf('.')+1);
		  
		  //remove all kind of '<date>.<weekDay>.'
		  dd['dsc'] = dd['dsc'].substring(dd['dsc'].indexOf('.',dd['dsc'].indexOf('.')+1)+1);
	  }
	  /*
      var wd = dd['weekDay'];
